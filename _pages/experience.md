...
### Knowledge Management Web Frontend Proxy Backend

Implemented the proxy backend for an AI web frontend that provides a ChatGPT-like interface for an internal knowledge management system. The platform features extensively customized tools tailored for internal staff use, serving as the primary interface for interacting with the company's knowledge base through AI.

### Multi-Source Data Crawling and Synchronization

Integrated the knowledge management system with multiple data sources to enable automatic resource crawling and synchronization for AI agent consumption. This included connecting to the internal wiki and media platforms, ensuring AI agents have access to up-to-date and comprehensive data across the organization's content ecosystem.
...
Developed two new capabilities for the AI platform:

- **Sound Effect Application**: Implemented a feature that applies sound effects to soundtracks using FFmpeg, such as radio or white noise background effects. The feature is controlled via JSON parameter configuration from the LLM, enabling context-aware audio enhancement.
- **Multitrack Mixer**: Built a mixing system that combines multiple audio tracks according to parameters specified either through JSON from the LLM or natural language input from the user, supporting complex audio composition workflows.
